...
angular.module('pfad').controller('EditFunctionController', function($scope, $routeParams, $location, flash, FunctionResource ) {
    ...
    $scope.get = function() {
        ...
        var errorCallback = function() {
            flash.setMessage({'type': 'error', 'text': 'The function could not be found.'});
            $location.path("/Functions");
        };
        FunctionResource.get({FunctionId:$routeParams.FunctionId}, successCallback, errorCallback);
    };
    ...
    $scope.save = function() {
        var successCallback = function(){
            flash.setMessage({'type':'success','text':'The function was updated successfully.'}, true);
            $scope.get();
        };
    ...
    $scope.remove = function() {
        var successCallback = function() {
            flash.setMessage({'type': 'error', 'text': 'The function was deleted.'});
            $location.path("/Functions");
        };
        var errorCallback = function(response) {
            if(response && response.data && response.data.message) {
