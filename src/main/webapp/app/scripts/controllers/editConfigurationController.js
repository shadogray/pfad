...
angular.module('pfad').controller('EditConfigurationController', function($scope, $routeParams, $location, flash, ConfigurationResource ) {
    ...
    $scope.get = function() {
        ...
        var errorCallback = function() {
            flash.setMessage({'type': 'error', 'text': 'The configuration could not be found.'});
            $location.path("/Configurations");
        };
        ConfigurationResource.get({ConfigurationId:$routeParams.ConfigurationId}, successCallback, errorCallback);
    };
    ...
    $scope.save = function() {
        var successCallback = function(){
            flash.setMessage({'type':'success','text':'The configuration was updated successfully.'}, true);
            $scope.get();
        };
    ...
    $scope.remove = function() {
        var successCallback = function() {
            flash.setMessage({'type': 'error', 'text': 'The configuration was deleted.'});
            $location.path("/Configurations");
        };
        var errorCallback = function(response) {
            if(response && response.data && response.data.message) {
